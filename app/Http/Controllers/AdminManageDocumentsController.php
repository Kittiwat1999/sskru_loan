...
class AdminManageDocumentsController extends Controller
{
    ...
    public function store_file_download(Request $request,$child_document_id){
        $rules = [
            'file_download' => 'required|file|mimes:jpg,png,jped,pdf|max:2048',
        ];
        $messages = [
            'file_download.required' => 'กรุณาเลือกไฟล์',
            'file_download.file' => 'ไฟล์ที่เลือกต้องเป็นไฟล์',
            'file_download.mimes' => 'ไฟล์ที่เลือกต้องเป็นประเภท: jpg, jpeg, png, pdf',
            'file_download.max' => 'ไฟล์ที่เลือกต้องมีขนาดไม่เกิน :max KB',
        ];

        $request->validate($rules,$messages);
        $child_document = ChildDocuments::find($child_document_id);
        $file_download = $request->file('file_download');
        $file_name = $this->storeFile($this->child_document_file_path,$file_download);
        ...
        $child_document_file['original_name'] = $file_download->getClientOriginalName();
        ...
        $child_document_file['full_path'] = 'app/public/'.$this->child_document_file_path.'/'.$file_name;
    ...
    public function store_example_file(Request $request,$child_document_id){
        $rules = [
            ...
            'minors_example_file' => 'file|mimes:jpg,jpeg,png,pdf|max:2048',
        ];
        $messages = [
            ...
            'minors_example_file.file' => 'ไฟล์ที่เลือกต้องเป็นไฟล์',
            'minors_example_file.mimes' => 'ไฟล์ที่เลือกต้องเป็นประเภท: jpg, jpeg, png, pdf',
            'minors_example_file.max' => 'ไฟล์ที่เลือกต้องมีขนาดไม่เกิน :max KB',
        ];

        $request->validate($rules,$messages);
        $child_document = ChildDocuments::find($child_document_id);
        $get_example_file = $request->file('file_everyone');
        $file_name = $this->storeFile($this->child_document_example_file_path,$get_example_file);
        $example_file = new ChildDocumentExampleFiles();
        $example_file['child_document_id'] = $child_document['id'];
        ...
        $example_file['file_path'] = $this->child_document_example_file_path;
        $example_file['file_name'] = $file_name;
        $example_file['file_type'] = last(explode('.', $file_name));
        $example_file['full_path'] = 'app/public/'.$this->child_document_example_file_path.'/'.$file_name;
        $example_file['upload_date'] = date('Y-m-d');
        $example_file->save();

        if($request->hasFile('file_example_minors')){
            $get_minors_example_file = $request->file('file_example_minors');
            $file_name = $this->storeFile($this->child_document_example_file_path,$get_minors_example_file);
            $minors_example_file = new ChildDocumentExampleFiles();
            $minors_example_file['child_document_id'] = $child_document['id'];
            $minors_example_file['description'] = 'ตัวอย่างเอกสารสำหรับผู้มีอายุต่ำกว่า 20 ปี';
            $minors_example_file['original_name'] = $get_minors_example_file->getClientOriginalName();
            $minors_example_file['file_for'] = 'minors';
            $minors_example_file['file_path'] = $this->child_document_example_file_path;
            $minors_example_file['file_name'] = $file_name;
            $minors_example_file['file_type'] = last(explode('.', $file_name));
            $minors_example_file['full_path'] = 'app/public/'.$this->child_document_example_file_path.'/'.$file_name;
    ...
    public function display_example_file_page($file_id){
        $example_file = ChildDocumentExampleFiles::find($file_id);
        return view('admin.display_example_file',compact('example_file'));
    }

    public function display_file_page($generate_file,$file_id){
        $file = ChildDocumentFiles::find($file_id);
        return view('admin.display_file',compact('generate_file','file'));
    }
